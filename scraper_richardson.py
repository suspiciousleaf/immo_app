...
def richardson_get_listings(host_photos=False):
    ...
    richardson_categories = ["vente-villa.cgi?000T", "vente-propriete.cgi?000T", "vente-maison-appartement.cgi?000T", "vente-terrain.cgi?000T", "investissement.cgi?000T", "vente-commerce.cgi?000T"]

    all_search_pages = []

    url_full = ["http://www.richardsonimmobilier.com/" + richardson_categories[i] for i in range(len(richardson_categories))]
    ...
    for object in resp_sold:
        if bool((BeautifulSoup(object["response"].content, "html.parser").find("span", class_="SIZE4"))) == False:
            links.append(object["link"])
            resp_to_scrape.append(object) # Save response objects to parse, to avoid scraping twice
