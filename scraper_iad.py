...
def iad_immo_get_listings(host_photos=False):
    ...
    all_search_pages = [f"https://www.iadfrance.fr/annonces/lavelanet-09300/vente?distance=20&locations%5B0%5D=Locality_43165&locations%5B1%5D=Locality_60619&locations%5B2%5D=Locality_56899&locations%5B3%5D=Locality_60477&locations%5B4%5D=Locality_45768&page={i}" for i in range(1, pages + 1)]
    ...
    listings = [listing for listing in listings_json if listing["agent"] == "IAD Immobilier"]
    ...
    if links_dead:
        ...
        for listing_ref in listing_photos_to_delete_local:
            try:
                shutil.rmtree(f'{cwd}/static/images/iad/{listing_ref}', ignore_errors=True) 
            except:
                pass
...
def get_listing_details(page, url, host_photos):
    ...
    try:
        ...
        # Price
        price = int(soup.find("div", class_="adPrice text-darkblue text-h3").get_text().replace("€", "").replace(" ", ""))
        ...
        details_div = soup.find("div", class_="adBadges flex mt-md wrap") # Many divs use the below class, so this search is done first to isolate the stat bar
        ...
        for item in details_div:
            if "m²" in item.get_text() and "terrain" not in item.get_text():
                size = int(float(("".join([x for x in item.get_text() if x.isnumeric() and x.isascii()]))))
                ...
                plot = int(float(("".join([x for x in item.get_text() if x.isnumeric() and x.isascii()]))))
        if plot:
            pass
        else:
            try:
                ...
                for result in plot_div:
                    ...
                    if "terrain" in div_contents:
                        ...
                        plot = int("".join([x for x in div_contents if x.isnumeric() and x.isascii()])) 
                        break
            except:
                pass
        ...
        for x in location_div[2:]:
            if x.isupper():
                ...
                break
        town = unidecode(location_div[first_upper_index + 2:location_div.find("(")-1]).replace("-", " ").capitalize()


        ref = soup.find("div", class_="mt-md text-grey-2 text-weight-medium").get_text().replace("Réf:", "").strip()
        types = link_url.replace("https://www.iadfrance.fr/annonce/", "").split("-")[0].capitalize()
        ...
        description = soup.find("div", class_="js-translatable-slots-content").get_text()
        # print(description)
