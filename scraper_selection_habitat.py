...
def selection_get_listings(host_photos=False):
    ...
    results_pages = [f"https://www.selectionhabitat.com/ajax/ListeBien.php?ope=1&page={i}&ListeViewBienForm=text&lieu=D%C2%A411%C2%A4Aude+(11)%C2%A40.7517563608706%C2%A40.0415626093272%C2%A40%7CD%C2%A409%C2%A4Ari%C3%A8ge+(09)%C2%A40.7493157360778%C2%A40.0251932867004%C2%A40%7CD%C2%A466%C2%A4Pyr%C3%A9n%C3%A9es-Orientales+(66)%C2%A40.7435327970408%C2%A40.0443244340435%C2%A40&lieu-alentour=0&langue=fr&MapWidth=100&MapHeight=394&DataConfig=JsConfig.GGMap.Liste&Pagination=0" for i in range(1, pages + 1)]
    ...
    listings = [listing for listing in listings_json if listing["agent"] == "Selection Habitat"]
    ...
    if links_dead:
        ...
        for listing_ref in listing_photos_to_delete_local:
            try:
                shutil.rmtree(f'{cwd}/static/images/selection/{listing_ref}', ignore_errors=True) 
            except:
                pass
    ...
    with concurrent.futures.ThreadPoolExecutor() as executor:   
        ...
        results = executor.map(get_listing_details, (item["response"] for item in response_objects), links_to_scrape, [host_photos for x in links_to_scrape]) 
        for result in results:
            if type(result) == str:
...
def selection_get_links(page):
    ...
    links = [link for link in links_raw if "https://www.selectionhabitat.com/fr/annonce/" in link]        
    # pprint(links)
    return links
...
def get_listing_details(page, url, host_photos):
    ...
    try:
        ...
        for element in details_list:
            if "Type" in element:
                types = element.replace("Type ", "").replace("agricole", "").replace("constructible", "").strip()
            elif "Ville" in element:
                town = unidecode(element.casefold().replace("ville ", "").replace("-", " ").replace("proche", "").strip()).capitalize()
                ...
                try:
                    size = int(float(element.replace("Surface ", "").replace("m²", "").strip())) # Float/int used in case of decimals
                except:
                    pass
            elif "Terrain" in element:
                try:
                    if "Hectare(s)" in element:
                        plot = int(float(element.replace("Terrain ", "").replace("m²", "").replace("Hectare(s)", "").strip()) * 10000)
                    else:    
                        plot = int(float(element.replace("Terrain ", "").replace("m²", "").strip()))
                except:
                    pass
