...
def c21_get_listings(host_photos=False):
    ...
    num_props_div = c21_soup.find("div", class_="tw-w-full md:tw-w-2/3 tw-text-base tw-font-medium tw-leading-none").get_text()
    ...
    all_search_pages = ["https://www.century21.fr/annonces/f/achat-maison-appartement-terrain-parking-immeuble-divers/d-09_ariege-11_aude/"]
    other_search_pages = [f"https://www.century21.fr/annonces/f/achat-maison-appartement-terrain-parking-immeuble-divers/d-09_ariege-11_aude/page-{i}/" for i in range(2, pages + 1)]
    ...
    listings = [listing for listing in listings_json if listing["agent"] == "Century 21"]
...
def c21_get_links(page):
    ...
    links = [f"https://www.century21.fr/{link}" for link in links_raw if "/trouver_logement/detail/" in link]    
...
def get_listing_details(page, url, host_photos):
    ...
    try:
        ...
        description_div = soup.find("div", class_="has-formated-text md:tw-pl-9").strings#get_text().strip()
        description = []
        ...
        try:
            ...
            # print(location_div)
            location_div = location_div.find("span", class_="tw-flex tw-flex-col md:tw-block md:tw-text-center md:tw-flex md:tw-flex-col").get_text().split("\n")
            # pprint(location_div)
                ...
                if item:
                    ...
                    for char in item:
                        if char.isnumeric():
                            ...
                            break
                    if not num:
                        town = unidecode(item.replace("-", " ").replace(" l ", " l'").capitalize())
                    elif num:
        ...
        # approx 3% of listings are missing the location bar at the bottom of the page. The section below goes through the description to see if any recognised town names are present, and sets location based on that. It will still miss hyphenated or multi-word town names, but given the rarity of those cases it's not worth coding in.
            ...
            town = None
            postcode = None
            description_joined = " ".join(description).replace(",", " ").replace(".", " ").replace("campagne", "").split()
            for word in description_joined:
                # print(word)
                if unidecode(word.casefold()) in town_list:
                    print(word.casefold())
                    town = unidecode(word.capitalize())
                    postcode = [i for i in postcodes_dict if town.casefold() in postcodes_dict[i]][0]
                    break        
        ...
        # Get price
        price = int(soup.find("div", class_="tw-flex tw-flex-col").get_text().strip().replace("€", "").replace(" ", ""))
        # print("Price:", price, "€")
        ...
        details_div = soup.find("section", class_="c-the-property-detail-global-view").get_text().replace(" ", "").split("\n")
        details_div = [unidecode(x.strip()) for x in details_div if x]  # Removes empty strings in list
        ...
        for item in details_div:
            if "Nombredepieces" in item:
                ...
                try:
                    plot = int(float(item.replace("Surfaceterrain : ", "").replace("m2", "").replace(",", ".")))
                except:
                    pass
            elif "Surfacetotale" in item:
                try:
                    size = int(float(item.replace("Surfacetotale : ", "").replace("m2", "").replace(",", ".")))
                except:
                    pass
