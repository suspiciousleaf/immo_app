...
def mm_immo_get_listings(host_photos=False):
    ...
    all_search_pages = [f"https://www.mmimmobilier.com/fr/annonces/acheter-p-r70-4-{i}.html" for i in range(1, pages + 1)]
    ...
    with concurrent.futures.ThreadPoolExecutor() as executor:  # Time elapsed for M&M Immo: 52.722009897232056 
...
def get_listing_details(page, url, host_photos):
    ...
    try:
        ...
        town = unidecode(location_town.replace("Region ", "").replace(" centre ville", "").replace("-", " ").replace("Val du faby", "Esperaza").replace("Quercorb", "Puivert"))
